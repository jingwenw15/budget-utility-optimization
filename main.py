...
'''
def quadratic_penalty_constraint_method(num_its=20, overbudget_penalty=1, spending_penalty=0.5): 
...
'''
For each item, we buy it with probability equal to threshold. 
Note that in this method, we can buy multiple of each item. 
TODO: WIP  
'''
def sample_point_multi(num_items, threshold=0.4): 
    # does not use previous distribution at all 
    pt = np.zeros((num_items, 1))
    for i in range(num_items): 
        keep_prob = random.random()
